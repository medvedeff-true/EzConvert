...
+ Windows 10/11 , Linux (`При условии что установлены все библиотеки из файла requirements.txt в проекте`)<br>
...
#### `У вас должен быть усновлен PyInstaller, если его нет, сначала установите его командой pip install pyinstaller`
**3.** В папке с проектом появится папка `dist`, в которой появится EzConvert.exe<br>
...
## English
# EzConvert
If you too are tired of constantly going to online converters when you need, for example, to convert a heic photo from an iPhone to a viable png, then EzConvert is especially for you!

***EzConvert*** is a program based on the Pillow library that allows you to convert pictures between the following formats: JPEG, PNG, BMP, TIFF, WEBP, ICO, GIF, TGA, HEIC.
...
## System Requirements
+ Windows 10/11 , Linux (`Provided that all libraries from requirements.txt file in the project are installed`)<br>
...
**1.** Download the project files and open them in a convenient editor.<br>
**2.** Compile and run `build.py` and wait for the build process to complete.<br>
#### `You must have PyInstaller installed, if not, install it first with pip install pyinstaller`.
**3.** A `dist` folder will appear in the project folder, in which EzConvert.exe will appear.<br>
